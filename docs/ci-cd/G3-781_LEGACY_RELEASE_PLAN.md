...
| **Help links — `5a712ad8`** | Click the header **?**, the footer **Help**, the frontpage Help panel and the quickstart PDF → each resolves to the monorepo docs site, not the retired `geneweaver-docs` site. ✅ **Prod gate RESOLVED (2026-09-17).** The repo and its Pages site are both public, so an anonymous visitor reaches the docs — verified by an unauthenticated fetch returning the real page (`<title>GeneWeaver</title>`), not a sign-in page. This row previously warned that the monorepo was internal and its Pages site org-only, which was true when written. The Help links shipped to Prod in 1.6.2. Was tracked as B9 in `docs/ci-cd/current-monorepo-cicd/LEGACY_CICD_MIGRATION.md` | |
