...
def test_login():
    ...
    with (
        patch("geneweaver.client.auth._get_device_code_data") as mock_get_device_code_data,
        patch(
            "geneweaver.client.auth._print_device_code_instructions"
        ) as mock_print_device_code_instructions,
        patch("geneweaver.client.auth._poll_for_flow_completion") as mock_poll_for_flow_completion,
        patch("geneweaver.client.auth.app_dir.save_auth_token") as mock_save_auth_token,
    ):
        ...
        mock_print_device_code_instructions.assert_called_once_with(mock_device_code_data)
