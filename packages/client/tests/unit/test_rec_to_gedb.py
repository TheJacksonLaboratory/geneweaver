...
class TestRecommenderDataStructures:
    ...
    @pytest.mark.usefixtures("test_client")
    def test_get_meta_for_tissue(self, test_client: GeneExpressionDatabaseClient) -> None:
        """Test meta mocked."""
        meta: list[Metadata] = test_client.get_meta("maxilla")
        assert meta is not None, "No metadata found for maxilla tissue!"

    def disabled_test_get_bulks_as_csv(self, test_client: GeneExpressionDatabaseClient) -> None:
        """Test bulks."""
        meta: list[Metadata] = test_client.get_meta("maxilla")
        ...
        assert len(expr_data.index) == 14767, f"The data frame size is {len(expr_data.index)}"
