...
def test_valid_url_req(mock_db_heath_service_call, client):
    ...
    response = client.get(url="/api/monitors/servers/health", params={"db_health_check": False})
    assert response.status_code == 200

    mock_db_heath_service_call.return_value = db_health_status.get("DB_status")
    response = client.get(url="/api/monitors/servers/health", params={"db_health_check": True})
    assert response.status_code == 200
    assert response.json().get("object").get("DB_status") == db_health_status.get("DB_status")
