...
"""
The majority of the following code was taken from py-upset found at https://github.com/ImSoErgodic/py-upset
This code is written for Python 3, so some modifications have been made to make the original Python code compatible
with Python 2.7 for integration into GeneWeaver.

** Denotes Modifications by Brandy Vickers **

"""


def plot(data_dict, **params):
    """
    ...
    ** Modified: Removed bare * in parameters. Replaced with params parameter as dictionary containing all possible
        named parameters used for this function **
    :param params: dictionary of parameters that may include none, any, or all of the following:

        unique_keys: list. Specifies the names of the columns that, together, can uniquely identify a row. If left
        ...
        sort_by: 'size' or 'degree'. The order in which to sort the intersection bar chart and matrix in the main
        graph

        inters_size_bounds: tuple. Specifies the size limits of the intersections that will be displayed. Intersections
        (and relative data) whose size is outside the interval will not be plotted. Defaults to (0, np.inf).

        inters_degree_bounds: tuple. Specified the degree limits of the intersections that will be displayed.
        Intersections (and relative data) whose degree is outside the interval will not be plotted.
    ...
    ** End modifications **
    ...
    # ** Modified **
    all_columns = params['unique_keys'] if 'unique_keys' in params else __get_all_common_columns(data_dict)
    sort_by = 'size' if 'sort_by' not in params else params['sort_by']
    inters_size_bounds = (0, np.inf) if 'inters_size_bounds' not in params else params['inters_size_bounds']
    inters_degree_bounds = (1, np.inf) if 'inters_degree_bounds' not in params else params['inters_degree_bounds']
    all_columns = list(all_columns)

    plot_data = DataExtractor(data_dict, all_columns)
    ordered_inters_sizes, ordered_in_sets, ordered_out_sets, ordered_gene_sets = \
    ...
    return ordered_dfs, ordered_df_names, ordered_in_sets, ordered_out_sets, ordered_inters_sizes, \
           ordered_gene_sets
...
class UpSetPlot():
    ...
    def additional_plot(self, ax_index, kind, data_values, graph_args, labels=None):
        ...
        ** Modified: Removed bare * parameter before labels **
...
class DataExtractor:
    def __init__(self, data_dict, unique_keys):
        ...
        self.out_sets_list, self.inters_df_dict, self.inters_genes = self.extract_intersection_data()
    ...
    def extract_intersection_data(self):
        ...
        for col_num, in_sets in enumerate(chain.from_iterable(
            ...
            genes_in_intersect.append(exclusive_intersection)

        return in_sets_list, inters_degrees, out_sets_list, inters_dict, genes_in_intersect
    ...
    def get_filtered_intersections(self, sort_by, inters_size_bounds, inters_degree_bounds):
        ...
        genes_sets_list = np.array(self.inters_genes)[size_clip]
        ...
        self.filtered_genes_sets = genes_sets_list[order]

        return self.filtered_inters_sizes, self.filtered_in_sets, self.filtered_out_sets, self.filtered_genes_sets
...
if __name__ == '__main__':
    # data_dict =
    # with open('./genesettest', 'r') as f:
    # ata_dict = load(f)
    df = DataFrame.from_dict({'geneId': (2, 3, 4, 5)})
    df2 = DataFrame.from_dict({'geneId': (4, 5, 7, 8)})
    df3 = DataFrame.from_dict({'geneId': (5, 6, 7)})
    #
    # data_dump = (json.dumps(data_dict))
    # return_dict = json.loads(data_dump)
    dic = {'gs1': df, 'gs2': df2, 'gs3': df3}
