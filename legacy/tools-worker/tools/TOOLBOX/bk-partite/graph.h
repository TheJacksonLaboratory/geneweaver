...
typedef struct graph_t {
  ...
  std::vector <int> partiteSizes;  /* size of each partite set */
  int numPartiteSets;
  std::vector <int> partiteSum;
  unsigned int* partiteIndex;
  std::vector <std::string> vertexLabels;
  std::vector <std::map <std::string, int> > vertexLabelsMap;
//  unsigned int hasPartiteMember;
  unsigned int mask;               /* xor with partiteBit returns 0 if all bits or set, otherwise non-zero */
  std::vector <int> whichPartiteSet;    /* stores which partite set each vertex is in */
  std::vector <int> cliquesize;  /* stores the size of each partite set in the current clique */
  int maxEdgeCliqueSize;
  int maxVertexCliqueSize;
  std::vector <int> maxEdgeClique;
  std::vector <int> maxVertexClique;
} Graph;
